...
if __name__ == "__main__":
    ...
    x_optimal_year = np.genfromtxt ('x_optimal_yearly.csv', delimiter=",")
    monthly_return_year = np.genfromtxt ('return_optimal_yearly.csv', delimiter=",")
    monthly_return_equal_year = np.genfromtxt ('return_equal_yearly.csv', delimiter=",")
    ...
    plt.plot(range(year_start - 1, year_end), net_value_optimal, label="3-factor model")
    ...
    plt.xlim((1990, 2015))
    plt.ylim((0, 150))
    plt.savefig('factor_cumulative.jpg', dpi=600)
    plt.show()
    plt.close("all")

    # plot the cumulative value including a transaction cost of 15 per year
    gain_log_optimal = monthly_return_year + np.ones((monthly_return_year.shape[0], monthly_return_year.shape[1]))
    gain_log_equal = monthly_return_equal_year + np.ones((monthly_return_equal_year.shape[0], monthly_return_equal_year.shape[1]))
    f = np.vectorize(math.log)
    g = np.vectorize(math.exp)
    gain_log_optimal = f(gain_log_optimal)
    gain_log_equal = f(gain_log_equal)
    yearly_gain_optimal = g(gain_log_optimal.sum(axis = 0)) - 0.01
    yearly_gain_equal = g(gain_log_equal.sum(axis = 0))
    yearly_gain_optimal_log = f(yearly_gain_optimal)
    yearly_gain_equal_log = f(yearly_gain_equal)
    net_value_optimal = g(np.cumsum(yearly_gain_optimal_log))
    ...
    plt.xlim((1990, 2015))
    plt.ylim((0, 150))
    plt.savefig('factor_cumulative_with_cost.jpg', dpi=600)
    plt.show()
    plt.close("all")
    
    # plot the "best return" vs the return of the equally distributed portfolio
    plt.plot(range(year_start, year_end), monthly_return_year.mean(axis = 0)/monthly_return_year.std(axis = 0), label="3-factor model")
    ...
    plt.savefig('factor_sr.jpg', dpi=600)
    plt.show()
    plt.close("all")


    # plot the "best return" vs the return of the equally distributed portfolio
    plt.plot(range(year_start, year_end), compounded_return_optimal, label="3-factor model")
    ...
    plt.xticks([1991, 1995, 2000, 2005, 2010, 2015])
    #plt.ylim((0, 50))
    plt.savefig('factor_c_return.jpg', dpi=600)
    plt.show()
    plt.close("all")

    # plot the "best return" vs the return of the equally distributed portfolio
    plt.plot(range(year_start, year_end), monthly_return_year.mean(axis = 0), label="3-factor model")
    ...
    plt.xlim((1991, 2015))
    plt.xticks([1991, 1995, 2000, 2005, 2010, 2015])
    plt.savefig('factor_monthly_return.jpg', dpi=600)
    plt.show()
    plt.close("all")

    # plot the "best return" vs the return of the equally distributed portfolio
    plt.plot(range(year_start, year_end), monthly_return_year.std(axis = 0), label="3-factor model")
    ...
    plt.xticks([1991, 1995, 2000, 2005, 2010, 2015])
    plt.ylim((0, 0.09))
    plt.savefig('factor_monthly_std.jpg', dpi=600)
    plt.show()
    plt.close("all")
